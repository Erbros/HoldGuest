...
class PlayerEventListener extends PlayerListener {
    ...
    public void onPlayerMove (PlayerMoveEvent event) {
	    ...
	    if(p.hasPermission("holdguest.leave")) {
        ...
        plugin.log.info("vec: " + plugin.vector.distanceSquared(p.getLocation().toVector()));
	    
	    // let's check if the player is inside the field? If he is, no reason to check more.
	    if(plugin.vector.distanceSquared(p.getLocation().toVector()) < plugin.holdradius) {
	        if(PlayerCache.getLocationCacheAge(p) > plugin.cacheage * 1000) {
                PlayerCache.setLocation(p, p.getLocation());
	    ...
	    if(PlayerCache.getLocation(p).toVector().distanceSquared(plugin.vector) < plugin.holdradius - 0.1) {
